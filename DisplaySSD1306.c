...
/*void SSD1306_Init(){
	...
	SSD1306_SendCommand(0xFF);
#else
	SSD1306_SendCommand(0xA8); //--set multiplex ratio(1 to 64) - CHECK
#endif
...
{
  SSD1306_SendCommand(SSD1306_RIGHT_HORIZONTAL_SCROLL);  // send 0x26
  SSD1306_SendCommand(0x00);  // send dummy
  SSD1306_SendCommand(start_row);  // start page address
  SSD1306_SendCommand(0X00);  // time interval 5 frames
  SSD1306_SendCommand(end_row);  // end page address
  SSD1306_SendCommand(0X00);
  SSD1306_SendCommand(0XFF);
  SSD1306_SendCommand(SSD1306_ACTIVATE_SCROLL); // start scroll
}

void SSD1306_ScrollLeft(uint8_t start_row, uint8_t end_row)
{
  SSD1306_SendCommand(SSD1306_LEFT_HORIZONTAL_SCROLL);  // send 0x26
  SSD1306_SendCommand(0x00);  // send dummy
  SSD1306_SendCommand(start_row);  // start page address
  SSD1306_SendCommand(0X00);  // time interval 5 frames
  SSD1306_SendCommand(end_row);  // end page address
